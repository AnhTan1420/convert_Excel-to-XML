...
# ==============================================================================
# FUNCTION 1: GENERATE RANDOM 5 SYNC RECORDS (4 TABLES)
# ==============================================================================
with st.expander("✨ Generate Random 5 Sync Records (4 Tables)", expanded=True):
    ...
    if st.button("Start Generating 5 Sample Data Records", type="primary", use_container_width=True):
        ...
        for s_id in (student_ids):
            ...
            ET.SubElement(item, 'STUDENT_NAME').text = f"RANDOM-NAME-{s_id}"
            ET.SubElement(item, 'HANYU_PINYIN_NAME').text = 'HANYUPINYIN'
            ET.SubElement(item, 'BIRTH_DATE').text = f"{random.randint(10, 28)}-{random.choice(['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'])}-{random.randint(2000, 2025)}"
            ...
            ET.SubElement(item, 'EMAIL_ADDRESS').set(f"{{{NS_XSI}}}nil", "true")
            ET.SubElement(item, 'CITIZENSHIP_EFFECTIVE_DATE').text = f"{random.randint(10, 28)}-{random.choice(['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'])}-{random.randint(2000, 2025)}"
        ...
        for s_id, p_id in zip(student_ids, parent_ids):
            ...
            ET.SubElement(item, 'RELATION_ICODE').text = random.choice(['9', 'F', '5', 'M', 'G2', 'G', 'G4', 'G3'])
            ET.SubElement(item, 'PARENT_GUARDIAN_NAME').text = f"Parent-Of-{s_id}"
            ET.SubElement(item, 'CITIZENSHIP_CODE').text = '10'
            ET.SubElement(item, 'RACE_CODE').text = '2'
            ...
            ET.SubElement(item, 'OTHER_CONTACT_NO').text = '1'
            ET.SubElement(item, 'BIRTH_DATE').text = f"{random.randint(10, 28)}-{random.choice(['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'])}-{random.randint(2000, 2025)}"
            ET.SubElement(item, 'EMAIL_ADDRESS').text = f"parent_{s_id.lower()}@yopmail.com"
            ET.SubElement(item, 'CITIZENSHIP_EFFECTIVE_DATE').text = f"{random.randint(10, 28)}-{random.choice(['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'])}-{random.randint(2000, 2025)}"
            ET.SubElement(item, 'CITIZENSHIP_SGDRM_CODE').text = 'SG'
        ...
        for d_id, p_id in zip(dif_student_ids, parent_ids):
            ...
            ET.SubElement(item, 'PARENT_UNIQUE_ID', {'UNIQUE_ID': 'Y'}).text = p_id
            ET.SubElement(item, 'RELATION_ICODE').text = random.choice(['F', 'M', 'G', 'G4'])
        ...
        # Pack all 4 compressed files into session state for user download
    ...
    if "sync_download_queue" in st.session_state:
        st.write("📂 Download the 4 linked sample data files:")
        cols = st.columns(4)
        for idx, item in enumerate(st.session_state["sync_download_queue"]):
            with cols[idx % 4]:
                st.download_button(
                    label=f"📥 {item['name'].split('_')[3]}", 
                    data=item['data'],
                    file_name=item['name'],
...
# ==============================================================================
# FUNCTION 2: GENERATE LARGE VOLUME DATASET (900,000 ROWS)
# ==============================================================================
st.markdown("---")
with st.expander("Generate 900K Records", expanded=False):
    st.write("Automatically generate `CUSTODIAL` structure data sequentially from 1 to 900,000 and compress it directly into a ZIP file to optimize memory footprint.")
    
    if st.button("Start Bulk Generation", type="secondary", use_container_width=True):
        progress_bar = st.progress(0)
        status_text = st.empty()
        
        status_text.text("⏳ Initializing compression pipeline stream...")
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for i in range(1, total_records + 1):
                ...
                if i % chunk_size == 0:
                    status_text.text(f"⏳ Processing data block: {i:,} / {total_records:,} records...")
        ...
        status_text.text("🎉 File generation complete! Ready for download.")
        progress_bar.empty()
    ...
    if "large_zip_data" in st.session_state:
        st.success(f"📦 Archive generation succeeded: `{st.session_state['large_zip_name']}`")
        st.download_button(
            label="📥 CLICK HERE TO DOWNLOAD ZIP PAYLOAD (900K DATA RECORDS)",
...
# ==============================================================================
# FUNCTION 3: CONVERT MANUALLY UPLOADED XLSX/CSV FILES
# ==============================================================================
...
with st.expander("📥 Download Excel Sample Templates", expanded=False):
    ...
    with col_t1: st.download_button(label="📁 ID_MAPPING", data=generate_template(TEMPLATE_COLUMNS["MAPPING"]), file_name="Template_ID_MAPPING.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with col_t2: st.download_button(label="📁 PERSONAL", data=generate_template(TEMPLATE_COLUMNS["PERSONAL"]), file_name="Template_BASIC_PERSONAL.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with col_t3: st.download_button(label="📁 SCHOOL", data=generate_template(TEMPLATE_COLUMNS["SCHOOL"]), file_name="Template_BASIC_SCHOOL.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with col_t4: st.download_button(label="📁 PARENT", data=generate_template(TEMPLATE_COLUMNS["PARENT"]), file_name="Template_STUDENT_PARENT.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with col_t5: st.download_button(label="📁 MOVEMENT", data=generate_template(TEMPLATE_COLUMNS["MOVEMENT"]), file_name="Template_MOVEMENT.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
    with col_t6: st.download_button(label="📁 CUSTODIAL", data=generate_template(TEMPLATE_COLUMNS["CUSTODIAL"]), file_name="Template_STUDENT_CUSTODIAL.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
...
uploaded_files = st.file_uploader("Drag and drop or browse Excel workbooks (.xlsx) or CSV files (.csv)", type=["xlsx", "csv"], accept_multiple_files=True)
    ...
    if st.button("🚀 Execute Split Transformation", type="primary", use_container_width=True):
        ...
        for uploaded_file in uploaded_files:
            ...
            if matched_rule:
                ...
                prefix = base_prefix.replace("FULL_", "DELTA_") if file_name.upper().startswith("DELTA_") else base_prefix
                ...
                try:
                    df = pd.read_csv(uploaded_file, encoding='utf-8') if file_name.lower().endswith(".csv") else pd.read_excel(uploaded_file, engine='openpyxl')
                    ...
                    root = ET.Element('INTERFACE', {'INTERFACE_NAME': interface, 'FILE_CREATED_TIME': str(int(time.time() * 1000)), 'FILE_NAME': xml_filename, 'NO_RECORD': str(len(df))})
                    ...
                    for index, row in df.iterrows():
                        ...
                        if is_personal: row_element = ET.SubElement(root, 'STUDENT_BASIC_PERSONAL')
                        elif is_school: row_element = ET.SubElement(root, 'STUDENT_BASIC_SCHOOL')
                        elif is_parent: row_element = ET.SubElement(root, 'STUDENT_PARENT')
                        elif is_movement: row_element = ET.SubElement(root, 'MOVEMENT')
                        elif is_custodial: row_element = ET.SubElement(root, 'STUDENT_CUSTODIAL_INFO_MK')
                        else: row_element = ET.SubElement(root, 'ID_Mapping', {'UNIQUE_ID': unique_id_val})
                        
                        for col_name in df.columns:
                            if not (is_personal or is_school or is_parent or is_movement or is_custodial) and col_name == 'UNIQUE_ID': continue
                            xml_tag = 'STUDENT_UNIQUE_ID' if col_name == 'UNIQUE_ID' and (is_personal or is_school or is_parent or is_movement or is_custodial) else col_name
                            
                            child = ET.SubElement(row_element, xml_tag)
                            val = row[col_name]
                            if xml_tag in ['STUDENT_UNIQUE_ID', 'PARENT_UNIQUE_ID']: child.set('UNIQUE_ID', 'Y')
                            
                            if val == "": child.set(f"{{{NS}}}nil", "true")
                            else: child.text = str(val)
                    ...
                    st.session_state.download_queue.append({
                        "zip_name": zip_filename, "zip_data": individual_zip_buffer.getvalue(), "source_name": file_name, "records": len(df)
                    })
                    success_count += 1
        ...
        st.session_state.run_summary = {"success_count": success_count, "total_records": total_records_processed}

    if st.session_state.download_queue:
        st.markdown("---")
        st.markdown("### Execution Run Summary")
        col1, col2 = st.columns(2)
        with col1: st.metric(label="Generated Payloads", value=f"{st.session_state.run_summary['success_count']} File(s)")
        with col2: st.metric(label="Total Dataset Records", value=f"{st.session_state.run_summary['total_records']} Rows")
