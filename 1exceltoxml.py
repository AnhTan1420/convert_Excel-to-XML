...
if uploaded_files:
    ...
    if st.button("🚀 Execute Workbook Transformation", type="primary", use_container_width=True):
        ...
        # --- PHASE 1: PRE-SCAN & BUILD LOOKUP DICTIONARY FROM ID MAPPING FILE ---
        student_id_lookup = {}  # Map NRIC/FIN -> Student UNIQUE_ID
        parent_id_lookup = {}   # Map NRIC/FIN -> Parent UNIQUE_ID
        
        mapping_file = None
        for f in uploaded_files:
            if "MAPPING" in f.name.upper():
                mapping_file = f
                break
        ...
        if mapping_file:
            try:
                ...
                # Dọn dẹp .0 cho các cột ID nếu có
                ...
                for _, row in df_map.iterrows():
                    ...
                    if uid:
                        if st_uin and st_uin != "":
                            student_id_lookup[st_uin] = uid
                        if pt_uin and pt_uin != "":
                            parent_id_lookup[pt_uin] = uid
                            
                st.toast("⚡ ID Mapping File detected! Auto-lookup dictionary compiled successfully.", icon="🔍")
            except Exception as ex:
                st.error(f"⚠️ Failed to parse ID Mapping file for lookups: {ex}")

        # --- PHASE 2: CONVERT ALL QUEUED FILES ---
            ...
            if matched_rule:
                ...
                try:
                    ...
                    root = ET.Element('INTERFACE', {
                        'INTERFACE_NAME': interface, 'FILE_CREATED_TIME': str(int(time.time() * 1000)),
                        'FILE_NAME': xml_filename, 'NO_RECORD': str(len(df)) 
                    })
                    
                    for index, row in df.iterrows():
                        id_col = 'UNIQUE_ID' if 'UNIQUE_ID' in df.columns else ('STUDENT_UNIQUE_ID' if 'STUDENT_UNIQUE_ID' in df.columns else None)
                        unique_id_val = str(row[id_col]).strip() if id_col and row[id_col] != "" else ""
                        
                        # --- AUTO-LOOKUP LOGIC FOR MISSING IDs ---
                        # 1. Dành cho bảng Personal / School (Cần Student ID)
                        if (is_personal or is_school) and (unique_id_val == ""):
                            st_uin_val = row.get("STUDENT_UIN_FIN_NO", "").strip()
                            if st_uin_val in student_id_lookup:
                                unique_id_val = student_id_lookup[st_uin_val]

                        # 2. Dành cho bảng Parent / Custodial / Movement (Cần cả Student ID & Parent ID)
                        parent_id_val = str(row.get("PARENT_UNIQUE_ID", "")).strip() if "PARENT_UNIQUE_ID" in df.columns else ""
                        if (is_parent or is_custodial or is_movement):
                            if unique_id_val == "":
                                # Nếu bảng Parent có cột NRIC của Student để map
                                st_uin_val = row.get("STUDENT_UIN_FIN_NO", "").strip()
                                if st_uin_val in student_id_lookup:
                                    unique_id_val = student_id_lookup[st_uin_val]
                            if parent_id_val == "":
                                # Map Parent ID qua cột PARENT_UIN_FIN_NO hoặc NRIC_POSTAL_ECODE nếu tận dụng cấu trúc cũ
                                pt_uin_val = row.get("PARENT_UIN_FIN_NO", "").strip() if "PARENT_UIN_FIN_NO" in df.columns else ""
                                if pt_uin_val in parent_id_lookup:
                                    parent_id_val = parent_id_lookup[pt_uin_val]
                        # ----------------------------------------
                        ...
                        for col_name in df.columns:
                            ...
                            # Gán value tương ứng (ưu tiên value đã được auto-lookup điền vào)
                            if xml_tag == 'STUDENT_UNIQUE_ID': val = unique_id_val
                            elif xml_tag == 'PARENT_UNIQUE_ID': val = parent_id_val
                            else: val = row[col_name].strip()
                    ...
                    st.session_state.download_queue.append({
                        "zip_name": zip_filename, "zip_data": individual_zip_buffer.getvalue(),
                        "source_name": file_name, "records": len(df)
                    })
                    success_count += 1
